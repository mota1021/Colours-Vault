...
この作業では、Jira上ではテスト可能な単位として分かれている **「拾うアクション」** と **「置くアクション」** を扱う。ただし、実際のBPアセットは分けず、**1 Actorにつき1個の `UColoursInteractableComponent` 派生Component**として実装する。
...
- 保持位置・カメラ方向への追従・角度制限
- 接地面角度による固定／滑り
- 落下中の再インタラクト禁止と接地後の再取得解禁
- 携帯ライト固有処理
- ライトの色
- 部屋への配置
- 持ち替え

「設置・再取得制御」は同じ `BPC_PickupPlace` を後から拡張して実装してよいが、Jira上では別Taskとして検証する。
